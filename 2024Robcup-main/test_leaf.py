...
def get_args_parser():
    ...
    # Dataset parameters
    # parser.add_argument('--data-path', default=os.path.join(os.path.expanduser("~"), 'Desktop\\imagenet'), type=str,
    #                     help='dataset path')
    ...
    parser.add_argument('--initial_checkpoint', type=str, default='pth/model_best.pth',
...
def main(args):
    ...
    ckpt.load("C:/Users/Kelun/Desktop/CrossViT-main/pth/model_best.pth")
    # read(model)
    ...
    #·
    #    image_path = "D:\classify\CrossViT-main\images\image" + str(count)+".jpg"
    #    # print(image_path)
    #
    #
    #    try:
        ...
        if ret:
            if text!=None and text!='':
                ...
                if text>"1]" :
                    ...
                    wenben = "Disease[" + text+"]"
                if text=="5]":
                    wenben="Disease["+text+"：叶黄病"
                elif text=="11":
                    wenben="Disease["+text+"]：锈病"
                elif text=="3]":
                    wenben="Disease["+text+"：霜霉病"

                frame = cv2AddChineseText(frame, wenben, (40, 40), (255, 0, 0), 30)
                # cv2.putText(frame, wenben, (40, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
                print(text)
                print(("----"))
            cv2.imshow('cap', frame)

            # img1 = cv2.imread("D:\classify\CrossViT-main\images\image1.jpg")
            # cv2.imshow('picture',img1)

            # if not ret: break
            if cv2.waitKey(1) & 0xFF == ord('w'):
           #      image_path = "C:/Users/Kelun/Desktop/CrossViT-main/img/image10.jpg"
           ...
           #  # 将OpenCV图像转换为RGB格式
                ...
                img_rgb = cv2.resize(img_rgb, (240, 240))
           # 将OpenCV图像转换为PIL Image
                ...
                test_stats = detect(data_loader_val, model)
                # print(f"已经读取到图片{count}")
                font = cv2.FONT_HERSHEY_SIMPLEX
                print(f"text是{text}")
                print(type(text))
                # cv2.waitKey(10000)
                cv2.putText(frame, text, (320, 240), font, 2, (255, 0, 0), 2)
...
def detect(data_loader, model):
        global max_indices,text
        ...
        if len(max_indices) == 10:
            text = max_indices[8:8]
            print(max_indices[8:8])
        else:
            text = max_indices[8:10:1]
            print(max_indices[8:10:1])
        # pub.publish(max_indices)
        # pub.loginfo("发布类成功")
...
if __name__ == '__main__':
    ...
    # pub = rospy.Publisher('depth_class',String,query_size = 10)
    main(args)
